...
$types=array('eot'=>'application/vnd.ms-fontobject','woff'=>'application/x-woff','ttf'=>'application/octet-stream');
if (!empty($_GET['type'])){
	if (isset($types[$_GET['type']],$_GET['name'])){$filename=$_GET['name'].'.' . $_GET['type'];
		...
		if(($clientLastModified && $clientEtag)?(($clientLastModified==$filemtime)&&($clientEtag==$etag)):($clientLastModified==$filemtime)){header("HTTP/1.1 304 Not Modified");exit;}
